...
const inputClass = 'h-12 border border-[color:rgba(9,20,19,0.12)] bg-white px-4 text-base font-medium text-[var(--slot4-page-text)] outline-none transition placeholder:text-[var(--slot4-soft-muted-text)] focus:border-[var(--slot4-accent-fill)]'
const buttonClass = 'inline-flex h-12 items-center justify-center border border-[var(--slot4-page-text)] px-6 text-sm font-semibold text-[var(--slot4-page-text)] transition hover:bg-[var(--slot4-page-text)] hover:text-white disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`px-4 py-3 text-sm font-medium ${status === 'success' ? 'bg-emerald-50 text-emerald-800' : 'bg-red-50 text-red-700'}`}>{message}</p> : null}
